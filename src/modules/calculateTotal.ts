import { IMonths, IStatiticsPerMonth } from "../types/IMonths";
import { IProduct, ITestProduct } from "../types/IProduct";
...
export const calculateTotal = (
  ...
  const total_price_saled = allProducts.reduce(
    (acc, product) => (acc += product.price_saled),
    0
  );
  const total_price_purchased = allProducts.reduce(
    (acc, product) => (acc += product.price_purchased),
    0
  );
  return {
    total_sales,
